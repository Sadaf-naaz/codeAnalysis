  ...
  return (
    <div className="login-container">
      <div className="login-card">
        <h2>Codespec - Code Quality Platform</h2>
...
function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('codespec-user');
    if (savedUser) {
  ...
  const handleLogin = (userData) => {
    setUser(userData);
    localStorage.setItem('codespec-user', JSON.stringify(userData));
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('codespec-user');
  };
